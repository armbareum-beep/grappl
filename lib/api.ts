...
export async function getCreators(): Promise<Creator[]> {
    try {
        ...
        // Optimized: Fetch all counts in 3 batch queries instead of N*3 queries
        ...
        const [coursesRes, routinesRes, sparringRes] = await Promise.all([
            ...
            supabase.from('routines').select('creator_id').in('creator_id', creatorIds),
            supabase.from('sparring_videos').select('creator_id').in('creator_id', creatorIds)
        ]);

        // Build count maps
        const courseCountMap = new Map<string, number>();
        const routineCountMap = new Map<string, number>();
        const sparringCountMap = new Map<string, number>();

        coursesRes.data?.forEach(c => {
            courseCountMap.set(c.creator_id, (courseCountMap.get(c.creator_id) || 0) + 1);
        });
        routinesRes.data?.forEach(r => {
            routineCountMap.set(r.creator_id, (routineCountMap.get(r.creator_id) || 0) + 1);
        });
        sparringRes.data?.forEach(s => {
            sparringCountMap.set(s.creator_id, (sparringCountMap.get(s.creator_id) || 0) + 1);
        });

        // Map creators with counts
        const creatorsWithCounts = creatorsData.map(creator => {
            return transformCreator(creator, {
                courseCount: courseCountMap.get(creator.id) || 0,
                routineCount: routineCountMap.get(creator.id) || 0,
                sparringCount: sparringCountMap.get(creator.id) || 0
            });
        });
