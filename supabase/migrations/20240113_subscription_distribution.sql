...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'video_watch_logs' AND column_name = 'drill_id') THEN
        ALTER TABLE video_watch_logs ADD COLUMN drill_id uuid REFERENCES drills(id);
    END IF;
END $$;
...
DECLARE
    ...
    -- Cursor for creator stats
    creator_stats CURSOR FOR
        SELECT 
            c.id AS creator_id,
            COALESCE(c.subscription_share, 0.8) as share_ratio, -- Default 0.8 if null
...
BEGIN
    ...
    -- 2. Calculate Total Platform Watch Time (for calculating ratios)
