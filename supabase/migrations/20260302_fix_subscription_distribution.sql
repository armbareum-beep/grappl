-- Fix: Also populate creator_payouts table when calculating distribution

-- 1. Ensure creator_payouts table exists
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'creator_payouts_creator_period_unique') THEN
        ALTER TABLE creator_payouts ADD CONSTRAINT creator_payouts_creator_period_unique
        UNIQUE (creator_id, payout_period_start);
    END IF;
EXCEPTION WHEN others THEN NULL;
END $$;

-- 3. Update the distribution function to also write to creator_payouts
...
BEGIN
    ...
    FOR creator_rec IN creator_stats LOOP
        ...
        IF creator_payout > 0 THEN
            ...
            -- Also insert into creator_payouts for UI display
