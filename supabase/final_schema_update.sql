...
BEGIN
    ...
    IF p_activity_type = 'like_received' THEN
        ...
        -- Cap at max
        v_xp_to_award := LEAST(v_xp_to_award, v_max_daily - v_daily_total);
    END IF;
    
    -- Award XP
    PERFORM add_xp(p_user_id, v_xp_to_award, p_activity_type);
    
    -- Record activity
    INSERT INTO xp_activities (user_id, activity_type, xp_earned)
    VALUES (p_user_id, p_activity_type, v_xp_to_award);
...
DROP TRIGGER IF EXISTS trigger_update_routine_stats ON routine_drills;
...
DROP TRIGGER IF EXISTS trigger_grant_course_routines ON user_courses;
...
DROP TRIGGER IF EXISTS trigger_grant_course_drills ON user_courses;
...
DROP POLICY IF EXISTS "Everyone can view routine drills" ON routine_drills;
...
DROP POLICY IF EXISTS "Creators can manage course routine bundles" ON course_routine_bundles;
...
DROP POLICY IF EXISTS "Everyone can view course routine bundles" ON course_routine_bundles;
...
DROP POLICY IF EXISTS "Creators can manage course drill bundles" ON course_drill_bundles;
...
DROP POLICY IF EXISTS "Everyone can view course drill bundles" ON course_drill_bundles;
...
DROP POLICY IF EXISTS "Users can view their own drill purchases" ON user_drill_purchases;
CREATE POLICY "Users can view their own drill purchases"
    ON user_drill_purchases FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can purchase drills" ON user_drill_purchases;
...
CREATE INDEX IF NOT EXISTS idx_sparring_reviews_date ON sparring_reviews(date DESC);
