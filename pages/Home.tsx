...
export const Home: React.FC = () => {
  ...
  // Mobile Tab State
  const [mobileTab, setMobileTab] = useState<'class' | 'drills' | 'sparring'>('class');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-24 md:pb-10">
      ...
      {/* Mobile: Drills Tab */}
      <div className={`md:hidden ${mobileTab === 'drills' ? 'block' : 'hidden'}`}>
        {mobileTab === 'drills' && <Drills />}
      </div>

      {/* Mobile: Sparring Tab */}
      <div className={`md:hidden ${mobileTab === 'sparring' ? 'block' : 'hidden'}`}>
        {mobileTab === 'sparring' && <SparringFeed />}
      </div>

      {/* Home Content (Visible on Desktop OR Mobile 'class' tab) */}
      <div className={`${mobileTab === 'class' ? 'block' : 'hidden'} md:block`}>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-900 border-t border-slate-800 pb-safe pt-2 px-6">
        <div className="flex justify-between items-center pb-2">
          <button
            onClick={() => setMobileTab('class')}
            className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-colors ${mobileTab === 'class' ? 'text-indigo-400' : 'text-slate-500'}`}
          >
            <BookOpen className={`w-6 h-6 ${mobileTab === 'class' ? 'fill-indigo-400/20' : ''}`} />
            <span className="text-[10px] font-bold">클래스</span>
          </button>

          <button
            onClick={() => setMobileTab('drills')}
            className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-colors ${mobileTab === 'drills' ? 'text-emerald-400' : 'text-slate-500'}`}
          >
            <Dumbbell className={`w-6 h-6 ${mobileTab === 'drills' ? 'fill-emerald-400/20' : ''}`} />
            <span className="text-[10px] font-bold">드릴 & 루틴</span>
          </button>

          <button
            onClick={() => setMobileTab('sparring')}
            className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-colors ${mobileTab === 'sparring' ? 'text-blue-400' : 'text-slate-500'}`}
          >
            <Sword className={`w-6 h-6 ${mobileTab === 'sparring' ? 'fill-blue-400/20' : ''}`} />
