...
import {
    ...
    transferBrandToCreator
} from '../../lib/api-organizers';
import { Event, EventType, Creator, EventBrand } from '../../types';
import { useToast } from '../../contexts/ToastContext';
import { ConfirmModal } from '../../components/common/ConfirmModal';
import { supabase } from '../../lib/supabase';

type MainTab = 'events' | 'teams';
type EventTabType = 'all' | EventType;

const EVENT_TABS: { key: EventTabType; label: string; icon: React.ElementType; color: string }[] = [
...
export const AdminEventList: React.FC = () => {
    ...
    // Main tab state
    const mainTab = (searchParams.get('tab') as MainTab) || 'events';

    // Events state
    const [events, setEvents] = useState<Event[]>([]);
    const eventTypeTab = (searchParams.get('type') as EventTabType) || 'all';
    ...
    const [eventTransferModal, setEventTransferModal] = useState<{
        isOpen: boolean;
        event: Event | null;
    ...
    // Team Transfer Modal
    const [teamTransferModal, setTeamTransferModal] = useState<{
        isOpen: boolean;
        team: EventBrand | null;
    ...
    const handleEventTypeChange = (type: EventTabType) => {
        if (type === 'all') {
            searchParams.delete('type');
        } else {
    ...
    const handleEventTransfer = async () => {
        ...
        try {
            ...
            setEventTransferModal({ isOpen: false, event: null });
    ...
    const handleTeamTransfer = async () => {
        if (!teamTransferModal.team || !selectedOrganizerId) return;

        try {
            setTransferring(true);
            await transferBrandToCreator(teamTransferModal.team.id, selectedOrganizerId);

            success('이벤트팀이 성공적으로 이전되었습니다.');
            setTeamTransferModal({ isOpen: false, team: null });
            setSelectedOrganizerId('');
            loadData();
        } catch (error) {
            console.error('Error transferring team:', error);
            toastError('이벤트팀 이전 중 오류가 발생했습니다.');
        } finally {
            setTransferring(false);
        }
    };
    ...
    const handleToggleTeamVisibility = async (team: EventBrand) => {
        try {
            await updateBrandVisibility(team.id, !team.isActive);
            success(team.isActive ? '이벤트팀이 숨김 처리되었습니다.' : '이벤트팀이 공개되었습니다.');
            loadData();
        } catch (error) {
        ...
        <div className="min-h-screen bg-zinc-950 text-white pb-20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className="flex gap-2 mb-6">
                    ...
                    <button
                        onClick={() => handleMainTabChange('teams')}
                        className={`flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all ${
                            mainTab === 'teams'
                                ? 'bg-amber-500 text-black'
                                : 'bg-zinc-900 text-zinc-400 hover:text-white border border-zinc-800'
                ...
                {mainTab === 'events' && (
                    <>
                        ...
                        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                            {EVENT_TABS.map(tab => {
                                const Icon = tab.icon;
                                const isActive = eventTypeTab === tab.key;
                                return (
                                    <button
                                        key={tab.key}
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-4 text-sm text-zinc-500 mt-1">
                                                    <span className="flex items-center gap-1">
                                                        ...
                                                        {new Date(event.eventDate).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}
                                                    </span>
                                                    {event.organizerName && (
                                                        <span className="flex items-center gap-1">
                                            ...
                                            <div className="flex items-center gap-2 w-full md:w-auto">
                                                ...
                                                <button
                                                    onClick={() => {
                                                        setEventTransferModal({ isOpen: true, event });
                                                        setSelectedOrganizerId('');
                                                    }}
                    ...
                    <>
                        ...
                        {filteredTeams.length === 0 ? (
                            <div className="text-center py-20 bg-zinc-900/30 rounded-3xl border border-dashed border-zinc-800">
                                ...
                                <h3 className="text-xl font-bold text-white mb-2">이벤트팀이 없습니다</h3>
                                <p className="text-zinc-500">등록된 이벤트팀이 없습니다.</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {filteredTeams.map((team) => (
                                    <div
                                        key={team.id}
                                        className={`bg-zinc-900/40 border rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center gap-4 backdrop-blur-sm group transition-all ${
                                            team.isActive
                                                ? 'border-zinc-800/50 hover:border-amber-500/20'
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-zinc-500 mt-1">
                                                ...
                                                <span className="flex items-center gap-1">
                                                    <Calendar className="w-3.5 h-3.5" />
                                                    {team.totalEvents}개 이벤트
                                                </span>
                                                <span className="flex items-center gap-1">
                                                    <Users className="w-3.5 h-3.5" />
                                                    {team.totalParticipants}명 참가
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 w-full md:w-auto flex-wrap">
                                            ...
                                            <button
                                                onClick={() => {
                                                    setTeamTransferModal({ isOpen: true, team });
                                                    setSelectedOrganizerId('');
                                                }}
                                                className="flex-1 md:flex-none flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 rounded-lg text-sm font-medium transition-all"
                                            >
                                            ...
                                            <button
                                                onClick={() => handleToggleTeamVisibility(team)}
                                                className={`flex-1 md:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                                                    team.isActive
            ...
            {/* Team Transfer Modal */}
            {teamTransferModal.isOpen && teamTransferModal.team && (
                <TransferModal
                    title="이벤트팀 이전"
                    description={`"${teamTransferModal.team.name}" 팀을 다른 주최자에게 이전합니다. 이 팀의 모든 이벤트도 함께 이전됩니다.`}
                    organizers={organizers}
                    currentOwnerId={teamTransferModal.team.creatorId}
                    selectedOrganizerId={selectedOrganizerId}
                    onSelectOrganizer={setSelectedOrganizerId}
                    onClose={() => setTeamTransferModal({ isOpen: false, team: null })}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-4">
                <div>
                    ...
                    <select
                        value={selectedOrganizerId}
                        onChange={(e) => onSelectOrganizer(e.target.value)}
                        className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-white focus:outline-none focus:border-amber-500/50"
                    >
