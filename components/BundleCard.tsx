...
export const BundleCard: React.FC<BundleCardProps> = ({ bundle }) => {
    ...
    const drillCount = bundle.drillIds?.length || 0;
    const totalItems = courseCount + drillCount;

    return (
        <Link
            to={`/checkout/bundle/${bundle.id}`}
            className="group block h-full relative rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 hover:border-blue-500/50 transition-all duration-300 hover:shadow-[0_20px_40px_-15px_rgba(59,130,246,0.15)] hover:-translate-y-1"
        >
