...
import { getPersonalizedLessons } from '../../lib/api-recommendations';
...
export const LessonsFeed: React.FC<{
    isEmbedded?: boolean;
    ...
    const loadLessons = async () => {
        try {
            ...
            const lessonsData = await getPersonalizedLessons(user?.id || null, 200);
            ...
            if (lessonsData) {
                ...
                const hotLessons = [...lessonsData]
                    .filter((l: any) => (l.views || 0) >= 5)
                    .sort((a, b) => getHotScore(b) - getHotScore(a));

                const processed = lessonsData.map((l: any) => {
                    const hotIndex = hotLessons.findIndex(s => s.id === l.id);
                    return {
                        ...l,
                ...
                // Personalized order is already applied
                setLessons(processed as Lesson[]);
            }
        } catch (err: any) {
    ...
    if (error) return (
        <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center">
            <div className="text-center">
                ...
                <button onClick={loadLessons} className="px-4 py-2 bg-violet-600 rounded-lg">재시도</button>
            </div>
        </div>
    );
    ...
    const filteredLessons = lessons.filter(l => {
        ...
        if (selectedCategory !== 'All') {
            matchesCategory = (l as any).category === selectedCategory;
        }
        ...
        <div className={cn(
            ...
            <div className={cn("max-w-[1600px] mx-auto w-full", isEmbedded && "px-0")}>
                ...
                <div className="flex flex-col gap-8 mb-8 mt-8">
                    {!isEmbedded && <h1 className="text-3xl font-bold text-white mb-2">레슨</h1>}
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
                        ...
                        <div className="text-zinc-500 text-sm font-medium">
                            총 <span className="text-zinc-200 font-bold">{filteredLessons.length}</span>개의 레슨
                        </div>
                    </div>
                    ...
                    {filteredLessons.map(lesson => (
                        ...
                        >
                            <div className="relative aspect-video rounded-xl overflow-hidden bg-zinc-900 mb-3">
                                <img
                                    src={getOptimizedThumbnail(lesson.thumbnailUrl, 'medium')}
                                    alt={lesson.title}
                            ...
                            <p className="text-xs text-zinc-500 truncate mt-1">
                                {lesson.courseTitle || 'Standalone Lesson'}
                            </p>
                        </div>
                    ))}
                </div>
