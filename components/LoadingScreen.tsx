...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({ message = '로딩 중...' }) => {
    ...
    const handleReset = () => {
        if (window.confirm('캐시를 삭제하고 새로고침 하시겠습니까?')) {
            // Clear all caches to fix loading issues
            localStorage.clear();
                    ...
                    <div className="flex flex-col items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800 max-w-xs">
                            <p className="text-slate-400 text-sm mb-3">
                                로딩이 지연되고 있나요? 캐시를 삭제하고 다시 시도해보세요.
                            </p>
                            <button
                                onClick={handleReset}
                                className="w-full px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-bold rounded-lg transition-colors shadow-lg"
                            >
                                캐시 삭제 및 새로고침
                            </button>
                        </div>
